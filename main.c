...
Eren Mermer, Sait Sevban Cander, Ugur Tekinalp
*/
#include <stdio.h>
#include <stdint.h>
#include "pieces.txt"
...
int colorinfo[8][8] = {   //keeps piece colors
    {2,2,0,0,0,0,1,1},
    {2,2,0,0,0,0,1,1},
    {2,2,0,0,0,0,1,1},
    {2,2,0,0,0,0,1,1},
	...
	int x,y;
	 for(x = 0; x < 8; x++){
        for(y = 0; y < 8; y++){
			...
			printsquare(x,y,colorinfo[x][y]-1,board[x][y]);
...
int LegalMove(){   //checks if it is a valid move or not
	...
	if(id==1){   //valid pawn moves
			...
			if(colorinfo[xfirst+1][yfirst-1])
	...
	if(id==2){   //valid rook moves
		for(i=1;i<=xfirst;i++){
			if(board[xfirst-i][yfirst]==0)
				...
				if(colorinfo[xfirst-i][yfirst]==pcol)
					break;
				moves[xfirst-i][yfirst]=1;
				break;
			}
		...
		for(i=1;i<=yfirst;i++){
			if(board[xfirst][yfirst-i]==0)
				...
				if(colorinfo[xfirst][yfirst-i]==pcol)
					break;
				moves[xfirst][yfirst-i]=1;
				break;
			}
	...
	if(id==3){  //valid bishop moves
		...
		for(i=1;i<=7;i++){
			if(board[xfirst+i][yfirst-i]==0)
				...
				if(colorinfo[xfirst+i][yfirst-i]==pcol)
					break;
				moves[xfirst+i][yfirst-i]=1;
				break;
			}
...
void flipBoard(){		//flips the board, gets the symmetric w.r.t. origin
	...
	for(i=0;i<4;i++){
		for(j=0;j<8;j++){
			...
			colorinfo[i][j] = colorinfo[7-i][7-j];
			board[7-i][7-j] = brd;
			colorinfo[7-i][7-j] = blw;
		}
	}
}
	...
	int tempcol;
		...
		if((moveFinish)%2==0){          // makes a move if the move is valid
			if((xsel!=xfirst)||(ysel!=yfirst)){
				...
				tempcol = colorinfo[xfirst][yfirst];   //stores the color of selected piece
				i=LegalMove();
				if((tempo)&&(i==1)){	//checks if the moving piece is a real piece and check if it is a legal move
					...
					colorinfo[xsel][ysel] = tempcol;	//writes stored piece to piece color array
					colorinfo[xfirst][yfirst]=0;		//clears the old square of the piece
				...
				if(i==2){		//same procedure for short castling 
					if(tempcol==1){
						...
						colorinfo[xfirst+1][yfirst] = tempcol;
						colorinfo[xfirst+3][yfirst]=0;
					}
					if(tempcol==2){
						board[xfirst-1][yfirst]=2;
						board[xfirst-3][yfirst]=0;
					...
					colorinfo[xsel][ysel] = tempcol;
					colorinfo[xfirst][yfirst]=0;
